...
func (h Handler) HTTPCheckerHandler(c *gin.Context) {
	...
	op := func() error {
		...
		// it's active — self-host has no per-ping analytics sink, so
		// monitor_status is the sole durable trace of a check. Drop the
		// transition guard (req.Status != "active") so a healthy-from-birth
		// monitor persists (and refreshes updated_at) on every tick.
